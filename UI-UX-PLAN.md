...
> SW cache `fittrack-v26`. Supabase is fully configured (schema through stage 7,
...
|---|---|
| 2026-08-21 (cont. 3) | **Coach header pinned** (user: the ＋ New / Past chats row scrolled away at the bottom of a long chat). The title row is now `.coach-head` — `position:sticky` at `calc(59px + safe-area-inset-top)` (the app-header's height), solid `--bg`, thin bottom border, full-bleed via `margin:0 -16px`, z-index between content and the app header. Check added: scrolled to bottom, `#coachNewBtn` is inside the viewport — coach 50/50, views green; screenshot verified at 390px. SW → `fittrack-v26`. |
| 2026-08-21 (cont. 2) | **Chat bubbles now wrap long unbroken strings** (user: the raw 400 error text forced the mobile page wider — had to side-scroll to reach Send). `.msg` gains `overflow-wrap:anywhere` + `word-break:break-word` + `min-width:0`; regression check in coach-test G injects the real 400 JSON and asserts no horizontal overflow at 390px — 49/49; views suite green. SW → `fittrack-v25`. Uncommitted. |
